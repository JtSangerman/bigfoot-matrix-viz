    ...
    class Program
    {
        static void Main(string[] args)
        {
			...
			if (args.Length != 3)
			{
				...
				Console.WriteLine("USAGE: \n Maze.exe <num rows> <num cols> <tick ms>");
				return;
			}

			int rows = 0, cols = 0, tickMs;

			try
			{
				//we check for possible errors: Parsing error, if rows/cols are negative or 0, or invalid # of args etc..
				rows = int.Parse(args[0]);
				cols = int.Parse(args[1]);
                                tickMs = int.Parse(args[2]);

				if (rows <= 0 || cols <= 0 || tickMs <= 0)
				{
					// if invalid argument or parsing error, throw exception
					Console.WriteLine("USAGE: \n Maze.exe <num rows> <num cols> <tick ms>");
					return;
				}
			}
			...
			{
				//catch the invalid format error and terminate
				Console.WriteLine("USAGE: \n Maze.exe <num rows> <num cols> <tickMs>");
				return;
			}

			//once we are sure the input is correct, begin building the maze!

			MazeHolder curMaze = new MazeHolder(rows, cols, tickMs);
		}
	}
				...
				if ((dirs = canMove(coord.getRow(), coord.getCol())).Length != 0)
				{
					...
					switch (dir)
					{
						...
						case 'L': //LEFT
							...
							maze[coord.getRow(), coord.getCol() - 1] = ' ';
							break;
						default: Console.WriteLine("check dir switch"); break; //(debugging)
					}
...
//			canvas = matrix.SwapOnVsync(canvas);
		...
		private bool solveMazeRecursively(char[,] maze, int x, int y, int d)
		{
			...
			if (lrow != -1)
			{
				...
				canvas.SetPixel(lrow, lcol, new Color(0, 0, 100));
			}
			lrow = x;
			lcol = y;
			if (visted[y, x])
                                ...
                                if (i != d)
				{
					...
					switch (i)
					{
						case 0:
							...
							{
								if (maze[y - 1, x] == ' ')
									//StdDraw.filledSquare(x, y - 1, ((double)scale) / (double)1.8);
									canvas.SetPixel(x, y - 1, c);
								ok = solveMazeRecursively(maze, x, y - 1, 2);
							}
							break;
						case 1:
							if ((x + 1 < maze.GetLength(0)) && maze[y, x + 1] == ' ')
							{
								if (maze[y, x + 1] == ' ')
							...
							break;
						case 2:
							if ((y + 1 < maze.GetLength(0)) && maze[y + 1, x] == ' ')
							{
								if (maze[y + 1, x] == ' ')
									//StdDraw.filledSquare(x, y + 1, ((double)scale) / (double)1.8);
									canvas.SetPixel(x, y + 1, c);
